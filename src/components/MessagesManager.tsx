  ...
  return (
    <div className="space-y-6 animate-in fade-in">
       ...
       <div className="grid gap-4">
          {contacts.map(c => (
             <div key={c.id} className={`p-6 rounded-3xl border card-shadow flex flex-col md:flex-row justify-between gap-6 transition-colors ${c.is_read ? 'bg-white border-slate-200' : 'bg-primary-50/50 border-primary-100'}`}>
                <div className="flex-1">
                   <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-bold text-lg">{c.name}</h3>
                      {!c.is_read && <span className="bg-primary-600 text-white px-2 py-0.5 rounded-full text-[10px] font-black uppercase">New</span>}
                   </div>
                ...
                <div className="flex flex-col gap-2 min-w-[150px]">
                   <button onClick={() => markRead(c.id, c.is_read)} className={`px-4 py-2 rounded-xl font-bold border transition-colors ${c.is_read ? 'bg-white border-slate-200 hover:bg-slate-50 text-slate-600' : 'bg-primary-100 border-primary-200 text-primary-700 hover:bg-primary-200'}`}>
                      {c.is_read ? 'Mark Unread' : 'Mark Read'}
                   </button>
                   <button onClick={() => handleDelete(c.id)} className="px-4 py-2 rounded-xl font-bold text-primary-600 hover:bg-primary-50 transition-colors">
                      Delete
                   </button>
                </div>
             </div>
